...
def taxa_de_conversao(df_olist_merge):
...
def taxa_de_conversao_por_origem(df_olist_merge):
...
taxa_de_conversao_por_origem(df_olist_merge)

print("-------------------------------------")


def taxa_de_conversao_por_pagina(df_olist_merge):
...
taxa_de_conversao_por_pagina(df_olist_merge)

print("-------------------------------------")


def receita_media_por_rs(df_olist_merge):
...
receita_media_por_rs(df_olist_merge)

print("-------------------------------------")

def business_type_convertido_por_rs(df_olist_merge):
...
business_type_convertido_por_rs(df_olist_merge)

print("-------------------------------------")

def lead_behaviour_profile_convertido_por_rs(df_olist_merge):
    df_olist_group = pd.DataFrame(
        df_olist_merge[df_olist_merge["fechou_acordo"] == "sim"].groupby(["sdr_id", "lead_behaviour_profile"])[
            "fechou_acordo"].count()).reset_index()
    df_olist_group.columns = ["sdr_id", "lead_behaviour_profile", "conversao"]
    print("Para cada SR, quantos de cada lead_behaviour_profile a pessoa converteu?\n", df_olist_group)

lead_behaviour_profile_convertido_por_rs(df_olist_merge)

print("-------------------------------------")


def lead_type_convertido_por_rs(df_olist_merge):
    df_olist_group = pd.DataFrame(
...
lead_type_convertido_por_rs(df_olist_merge)
